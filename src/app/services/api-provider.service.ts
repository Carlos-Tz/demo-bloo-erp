...
export class ApiProviderService {
  ...
  AddProvider(provider: Provider) {
    ...
    this.db.database.ref().child('blooming-erp/provider-list/'+ provider.id).once("value", snapshot => {
      if(snapshot.exists()){
        this.toastr.error('No guardado, ya existe un registro con este nombre!');
        return true;
      }
      //this.toastr.success('Guardado!');
      this.db.database.ref().child('blooming-erp/provider-list/'+ provider.id).set(provider);
      return false;
    });
  }

  GetProviderList() {
    this.providerList = this.db.list('blooming-erp/provider-list');
    return this.providerList;
  }

  GetProvider(key: string) {
    this.providerObject = this.db.object('blooming-erp/provider-list/' + key);
    return this.providerObject;
  }

  UpdateProvider(provider: Provider, key: string) {
    this.db.object('blooming-erp/provider-list/' + key)
    .update(provider);
  }
  ...
  GetLastProvider(){
    this.lastProviderRef = this.db.list('blooming-erp/provider-list/', ref => ref.limitToLast(1)).valueChanges();
