...
export class ApiCategoryService {
  ...
  AddCategory(category: Category) {
    ...
    this.db.database.ref().child('blooming/category-list/'+ category.id).once("value", snapshot => {
      if(snapshot.exists()){
        this.toastr.error('No guardado, ya existe un registro con este nombre!');
        return true;
      }
      //this.toastr.success('Guardado!');
      this.db.database.ref().child('blooming/category-list/'+ category.id).set(category);
      return false;
    });
  }
  
  GetCategoryList() {
    this.categoryList = this.db.list('blooming/category-list')
    return this.categoryList;
  }
  
  GetCategory(key: string) {
    this.categoryObject = this.db.object('blooming/category-list/' + key);
    return this.categoryObject;
  }

  UpdateCategory(category: Category, key: string) {
    this.db.object('blooming/category-list/' + key)
    .update(category);
  }
  ...
  GetLastCategory(){
    this.lastCategoryRef = this.db.list('blooming/category-list/', ref => ref.limitToLast(1)).valueChanges();
